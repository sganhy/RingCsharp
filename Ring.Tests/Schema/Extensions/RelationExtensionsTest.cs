...
public class RelationExtensionsTest : BaseExtensionsTest
{
    ...
    public void ToMeta_Relation1_MetaObject()
    {
        ...
        Assert.Equal(1054, meta.GetEntityRefId());
        Assert.Equal(1021, meta.DataType);
        Assert.Equal("test=test", meta.GetEntityDescription());
        Assert.Equal(524304, meta.Flags);
        Assert.Equal("rule2book", meta.GetEntityName());
    ...
    public void ToMeta_Relation2_MetaObject()
    {
        ...
        Assert.Equal(1, meta.GetEntityId());
        Assert.Equal(EntityType.Relation, meta.GetEntityType());
        Assert.Equal(1013, meta.GetEntityRefId());
        Assert.Equal(1012, meta.DataType);
        Assert.Equal(786448, meta.Flags);
        Assert.Equal("align_descriptor2alignment", meta.GetEntityName());
    ...
    internal void SetInverseRelation_Meta_RelationObject()
    {
        ...
        var relation1 = MetaExtensions.GetEmptyRelation(meta1, RelationType.Otm);
        var relation2 = MetaExtensions.GetEmptyRelation(meta2, RelationType.Mtm);
        var relation3 = MetaExtensions.GetEmptyRelation(meta3, RelationType.Mto);
    ...
    internal void Initialized_Relation2_False()
    {
        ...
        var meta3 = new Meta(_fixture.Create<string>());
        var relation2 = MetaExtensions.GetEmptyRelation(meta2, RelationType.Mtm);
        var relation3 = MetaExtensions.GetEmptyRelation(meta3, RelationType.Mtm);
    ...
    internal void Initialized_Relation3_True()
    {
        ...
        var relation3 = MetaExtensions.GetEmptyRelation(meta3, RelationType.Mtm, TableType.Mtm);
        var relation4 = MetaExtensions.GetEmptyRelation(meta4, RelationType.Mtm);
