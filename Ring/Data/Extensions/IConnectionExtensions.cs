...
internal static class IConnectionExtensions
{
	...
	private static readonly ConcurrentDictionary<int, Counter> _counterByConnectionHash = new();
	...
	/// </summary>
	internal static long GetId(this IConnection _, int connParametersHash)
	{
		// Code size: 82 (0x52)
		// Fast path: lock-free read — the common case once all connection
		// strings have been seen at least once (e.g. after warm-up).
		if (_counterByConnectionHash.TryGetValue(connParametersHash, out var counter))
		...
		counter = _counterByConnectionHash.GetOrAdd(connParametersHash, _ => new Counter());
		return Interlocked.Increment(ref counter.Value);
	}
}
