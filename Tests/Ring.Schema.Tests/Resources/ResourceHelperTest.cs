...
public sealed class ResourceHelperTest
{
    ...
    public void GetSchemaTemplate_NativeXmlTemplateType_Ok()
    {
        ...
        Assert.Equal(4, template.MaxDepth);
        ...
        Assert.Equal(8, template.Items[1].Attributes.Length);
	...
	[Theory]
	[InlineData(LogType.FileNotFound, "File '{0}' not found.",  LogLevel.Error)]
	[InlineData(LogType.FileToolLarge, "File size exceeds the maximum allowed size of {0} bytes.", LogLevel.Error)]
