...
public sealed class MetaTest : BaseTest
{
    ...
    [InlineData(17, FieldType.Date)]
    [InlineData(18, FieldType.DateTime)]
    [InlineData(19, FieldType.DateTimeOffset)]
	...
	[Fact]
	internal void SetFieldSize_MaxInt32_ReturnMaxInt32()
	{
		// arrange 
		var size = int.MaxValue; // min value == 0
		var flags = 0L;
		...
		var meta = new Meta(_faker.Random.Number(int.MinValue, int.MaxValue), (byte)EntityType.Schema,
			_faker.Random.Number(int.MinValue, int.MaxValue), _faker.Random.Number(int.MinValue, int.MaxValue),
			flags + 1001 + 0x100000000000000, _faker.Random.String(), _faker.Random.String(), _faker.Random.String(), false);

		// act 
		var result = meta.GetFieldSize();
	...
	[Fact]
	internal void SetFieldSize_0_Return0()
	{
		// arrange 
		var size = 0; // min value == 0
		var flags = 0L;
		flags = Meta.SetFieldSize(flags, size);
		var meta = new Meta(_faker.Random.Number(int.MinValue, int.MaxValue), (byte)EntityType.Schema,
			_faker.Random.Number(int.MinValue, int.MaxValue), _faker.Random.Number(int.MinValue, int.MaxValue),
    ...
    internal void ToField_Meta1_FieldObject()
    {
        ...
        flags = Meta.SetFieldSize(flags, 80);
        flags = Meta.SetFieldNotNull(flags, true);
		flags = Meta.SetEntityBaseline(flags, true);
		flags = Meta.SetFieldMultilingual(flags, true);
		var meta = new Meta(2, (byte)EntityType.Field, 1011, 16, flags, "name",_faker.Random.String(), null, true);
    ...
    internal void ToField_Meta2_FieldObject()
    {
		...
		flags = Meta.SetSearchableType(flags, SearchableType.IgnoreCase);
		flags = Meta.SetFieldSize(flags, exepectedFieldSize);
		flags = Meta.SetFieldNotNull(flags, false);
		flags = Meta.SetEntityBaseline(flags, false);
		flags = Meta.SetFieldMultilingual(flags, true);
		var meta = new Meta(6, (byte)EntityType.Field, 1021, 16, flags, "isbn", _faker.Random.String(), "Test2", false);
    ...
    internal void ToField_Meta3_FieldObject()
    {
		...
		flags = Meta.SetSearchableType(flags, SearchableType.IgnoreDiacritic);
		flags = Meta.SetFieldSize(flags, exepectedFieldSize);
		flags = Meta.SetFieldNotNull(flags, false);
		flags = Meta.SetEntityBaseline(flags, false);
		flags = Meta.SetFieldMultilingual(flags, false);
		var meta = new Meta(4, (byte)EntityType.Field, 1032 , 2, flags, "status", _faker.Random.String(), "Test3", true);
        ...
        Assert.Equal(field.Size, exepectedFieldSize);
        Assert.True(field.Active);
        Assert.Equal(SearchableType.IgnoreDiacritic, field.SearchableType);
        Assert.False(field.NotNull);
        Assert.False(field.Baseline);
    ...
    internal void ToField_Meta5_FieldObject()
    {
        ...
        flags = Meta.SetSearchableType(flags, SearchableType.IgnoreDiacritic);
        flags = Meta.SetEntityBaseline(flags, false);
		flags = Meta.SetFieldSize(flags, 22); //Test:  doesn't clear existing bits before writing.
    ...
    internal void ToRelation_Meta1_RelationObject()
    {
        ...
        var meta = new Meta(2, (byte)EntityType.Relation, 1011, _faker.Random.Number(int.MinValue,int.MaxValue), 3145744L, "ability2book", _faker.Random.String(), "book2ability", true);
    ...
    internal void ToParameter_Meta1_ParameterObject()
    {
        ...
        var meta = new Meta(4, (byte)EntityType.Parameter, 0, 16, 286720L, "@language", _faker.Random.String(), expectedValue, true);
    ...
    internal void ToTable_Meta1_TableObject()
    {
        ...
        Assert.Equal(8, table.Columns.Length);
    ...
    {
        // arrange 
        var meta1 = Meta.Create("meta1");
        var meta2 = Meta.Create("meta2");
